...
[**getMyCustomerInfos()**](CustomersApi.md#getMyCustomerInfos) | **GET** /customers/me | 
...
```php
getMyCustomerInfos(): \Jalismrs\Stalactite\Client\Data\Model\Customer
```
...
try {
    $result = $apiInstance->getMyCustomerInfos();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling CustomersApi->getMyCustomerInfos: ', $e->getMessage(), PHP_EOL;
}
```
